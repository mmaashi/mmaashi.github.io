...
import { ShieldCheck, TrendingUp, TrendingDown, Minus, BarChart3, ArrowUpRight } from "lucide-react";
  ...
  return (
    <div className="card" style={{ padding: "20px 22px" }}>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span
          style={{
            fontSize: 10, fontWeight: 700, color: d.overallColor,
            padding: "3px 10px", borderRadius: 6,
            background: `${d.overallColor}15`, border: `1px solid ${d.overallColor}25`,
          }}
        >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
        {d.dimensions.map((dim) => (
          <div key={dim.key} className="flex items-center gap-3">
            <span style={{ fontSize: 10, color: "var(--c-muted)", width: 90, flexShrink: 0, fontWeight: 600 }}>
            ...
            <span style={{ fontSize: 9, fontWeight: 700, color: signalColor(dim.signal), width: 44, textAlign: "right", flexShrink: 0 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
        {/* Strengths */}
        <div>
          <span style={{ fontSize: 9, fontWeight: 700, color: "var(--c-green)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 6, display: "block" }}>
            {isAr ? "ما يبدو قويًا" : "What looks strong"}
          </span>
          {d.strengths.map((s, i) => (
            <div key={i} className="flex items-start gap-2" style={{ marginBottom: 5 }}>
              <div style={{ width: 4, height: 4, borderRadius: "50%", background: "var(--c-green)", marginTop: 5, flexShrink: 0 }} />
              <p style={{ fontSize: 10, color: "var(--c-text-sm)", lineHeight: 1.45, margin: 0 }}>
                {isAr ? s.ar : s.en}
              </p>
            </div>
          ))}
        ...
        {/* Watchouts */}
        <div>
          <span style={{ fontSize: 9, fontWeight: 700, color: "var(--c-gold)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 6, display: "block" }}>
            {isAr ? "ما يستحق المراقبة" : "What to watch"}
          </span>
          {d.watchouts.map((w, i) => (
            <div key={i} className="flex items-start gap-2" style={{ marginBottom: 5 }}>
              <div style={{ width: 4, height: 4, borderRadius: "50%", background: "var(--c-gold)", marginTop: 5, flexShrink: 0 }} />
              <p style={{ fontSize: 10, color: "var(--c-text-sm)", lineHeight: 1.45, margin: 0 }}>
                {isAr ? w.ar : w.en}
              </p>
            </div>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 10, color: "var(--c-muted)", margin: 0, marginTop: 2, lineHeight: 1.4 }}>
        ...
        {d.benchmarkReturn !== null && d.tasiReturn !== null && (
          <div className="flex items-center gap-3">
            <div style={{ textAlign: "center" }}>
              <span style={{ fontSize: 8, color: "var(--c-dim)", display: "block" }}>{isAr ? "أنت" : "You"}</span>
              <span className="font-num font-bold" style={{ fontSize: 12, color: d.benchmarkReturn >= 0 ? "var(--c-green)" : "var(--c-red)" }}>
                {d.benchmarkReturn >= 0 ? "+" : ""}{d.benchmarkReturn.toFixed(1)}%
              </span>
            </div>
            <div style={{ textAlign: "center" }}>
              <span style={{ fontSize: 8, color: "var(--c-dim)", display: "block" }}>TASI</span>
              <span className="font-num font-bold" style={{ fontSize: 12, color: d.tasiReturn !== null && d.tasiReturn >= 0 ? "var(--c-green)" : "var(--c-red)" }}>
                {d.tasiReturn >= 0 ? "+" : ""}{d.tasiReturn.toFixed(1)}%
              </span>
            </div>
          </div>
        )}
