...
import { TrendingUp, TrendingDown, Sparkles, ShieldCheck, Plus, Search } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: "1 1 280px", minWidth: 0 }}>
        <p style={{ fontSize: 15, fontWeight: 700, color: "var(--c-text)", marginBottom: 2, fontFamily: "var(--font-grotesk)" }}>
          {isAr ? "مرحبًا" : "Good morning"} 👋
        </p>
        <p style={{ fontSize: 11, color: "var(--c-muted)", marginBottom: 18, lineHeight: 1.55 }}>
        ...
        <div className="flex items-center gap-3" style={{ flexWrap: "wrap" }}>
          <button
            style={{
              ...
              color: "var(--c-muted)", fontSize: 11, fontWeight: 600, cursor: "pointer",
            }}
          >
            <Plus size={11} />
            {isAr ? "أضف للمحفظة" : "Add portfolio"}
          </button>
          <Link
            ...
            style={{
              ...
              color: "var(--c-gold)", fontSize: 11, fontWeight: 700, textDecoration: "none",
            }}
          >
            <Search size={11} />
            {isAr ? "استكشف الأسهم" : "Explore matches"}
          </Link>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "flex-start" }}>
        ...
        <div>
          ...
          <div className="font-num font-bold" style={{ fontSize: 28, color: "var(--c-text)", lineHeight: 1.15, marginTop: 2 }}>
        ...
        <div>
          ...
          <div className="flex items-center gap-1" style={{ marginTop: 4 }}>
            {up ? <TrendingUp size={13} style={{ color: "var(--c-green)" }} /> : <TrendingDown size={13} style={{ color: "var(--c-red)" }} />}
            <span className="font-num font-bold" style={{ fontSize: 15, color: up ? "var(--c-green)" : "var(--c-red)" }}>
              {up ? "+" : ""}{d.dailyChange.toFixed(2)}%
            </span>
          </div>
          <span className="font-num" style={{ fontSize: 10, color: "var(--c-dim)" }}>
            {up ? "+" : ""}{fmtCurrency(Math.abs(d.dailyChangeAmount), d.sar)}
          </span>
        </div>
        ...
        <div>
          ...
          <div className="font-num font-bold" style={{ fontSize: 15, color: totalUp ? "var(--c-green)" : "var(--c-red)", marginTop: 4 }}>
            {totalUp ? "+" : ""}{d.totalReturn.toFixed(1)}%
          </div>
          <span className="font-num" style={{ fontSize: 10, color: "var(--c-dim)" }}>
            {totalUp ? "+" : ""}{fmtCurrency(Math.abs(d.totalReturnAmount), d.sar)}
          </span>
        </div>
        ...
        {d.weightedScore !== null && (
          <div>
            ...
            <div className="flex items-center gap-1.5" style={{ marginTop: 4 }}>
              ...
              <span className="font-num font-bold" style={{ fontSize: 17, color: scoreColor(d.weightedScore) }}>
