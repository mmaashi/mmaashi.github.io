        ...
        <ResponsiveContainer width="100%" height={190}>
          <BarChart data={sorted} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="revenue" name="Revenue" radius={[5, 5, 0, 0]}>
              {sorted.map((_, i) => (
                <Cell key={i} fill={i === sorted.length - 1 ? "rgba(200,169,81,1)" : "rgba(200,169,81,0.38)"} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
            ...
            <Bar dataKey="netIncome" name="Net Income" radius={[5, 5, 0, 0]}>
              {sorted.map((entry, i) => (
                <Cell
                  ...
                  fill={
                    (entry.netIncome ?? 0) < 0
                      ? i === sorted.length - 1 ? "rgba(246,70,93,1)" : "rgba(246,70,93,0.38)"
                      : i === sorted.length - 1 ? "rgba(14,203,129,1)" : "rgba(14,203,129,0.38)"
                  }
                />
              ))}
            </Bar>
