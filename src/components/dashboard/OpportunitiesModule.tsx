...
import { Sparkles, TrendingUp, DollarSign, ShieldCheck, Eye } from "lucide-react";
...
// TRUST-FIRST labels — no overclaiming
const tagConfig: Record<
  string,
  { icon: typeof Sparkles; color: string; bg: string; labelEn: string; labelAr: string }
> = {
  undervalued: {
    icon: DollarSign,
    color: "var(--c-green)",
    bg: "rgba(34,197,94,0.05)",
    labelEn: "Attractive valuation",
    labelAr: "تقييم جاذب",
  },
  high_quality: {
    icon: ShieldCheck,
    color: "var(--c-gold)",
    bg: "rgba(200,169,81,0.05)",
    labelEn: "High-quality candidate",
    labelAr: "مرشح عالي الجودة",
  },
  dividend_leader: {
    icon: Sparkles,
    color: "#a78bfa",
    bg: "rgba(167,139,250,0.05)",
    labelEn: "Strong income profile",
    labelAr: "ملف دخل قوي",
  },
  momentum: {
    icon: TrendingUp,
    ...
    labelEn: "Quality with improving momentum",
    labelAr: "جودة مع تحسن الزخم",
  },
};
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div className="flex items-center gap-2 mb-1">
        ...
        >
          {isAr ? "أفكار تناسب أسلوبك" : "Ideas that fit your style"}
        </h3>
      </div>
      <p style={{ fontSize: 10, color: "var(--c-dim)", marginBottom: 14 }}>
        {isAr
          ? "أفكار مبنية على فلاترك وقائمة متابعتك — ليست توصيات استثمارية"
          : "Based on your screens and watchlist — not investment advice"}
      </p>

      {opportunities.length === 0 ? (
        <div className="card" style={{ padding: "24px", textAlign: "center" }}>
          <p style={{ fontSize: 11, color: "var(--c-dim)" }}>
            {isAr ? "لا توجد فرص حاليًا" : "No opportunities right now"}
          </p>
        </div>
      ) : (
        <div
